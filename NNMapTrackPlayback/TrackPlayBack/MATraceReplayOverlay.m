...
- (instancetype) initWithLocations: (NSArray <CLLocation *>*) locations {
    if (self = [super init]) {
        ...
        [self setWithLocations:locations];
    }
    return self;
}
...
- (void) setWithLocations: (NSArray <CLLocation *>*) locations {
    MAMapPoint *p = (MAMapPoint *)malloc(locations.count * sizeof(MAMapPoint));
    NSInteger index = 0;
    ...
    [self setWithPoints:p count:locations.count];
    if(p) {
        free(p);
    }
}
...
- (void)drawStepWithTime:(NSTimeInterval)timeDelta zoomLevel:(CGFloat)zoomLevel {
    ...
    if(!_isPaused || !_readyForDrawing || hasRecalculated) {
        ...
        if(curIndex >= (_origPointCount - 1)) {
            ...
            _multiPolyline.drawStyleIndexes = @[@(0), @(_reducedPointIndexOfCar)];
        ...
        if(_carIndexInOrigArray == 0) {
            ...
            //更新patchline
            _patchLinePoints[0] = p1;
            _patchLinePoints[1] = _carMapPoint;
            [_patchLine setPolylineWithPoints:_patchLinePoints count:2];
        }
        
        /// 车辆方向更新回调
        if (self.callBackDirection) {
            self.callBackDirection();
        }
    }
